  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch('/api/leads', {
        ...
        body: JSON.stringify({ ...form, source: 'signin' }),
      });
      ...
      if (!res.ok) {
        ...
        setStatus('success');
        setMessage('Access request received. A link will be sent to your work email shortly.');
      ...
      <div className="w-full max-w-md">
        ...
        {status === 'success' ? (
          ...
          <div className="bg-[#1A2540] border border-[#1E2D4A] rounded-xl p-8">
            <h1 className="text-white font-semibold text-2xl mb-1">Sign in</h1>
            ...
            <p className="mt-6 text-center text-xs text-[#8A99B8]">
              Not yet registered?{' '}
              <a href="/sign-up" className="text-[#1D6FF2] hover:underline">
                Request access
              </a>
            </p>
          </div>
