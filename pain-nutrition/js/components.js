/* ─────────────────────────────────────────────────────────────────────────────
   Pain Nutrition — Components  (PNComponents namespace)
   Same API as FHIComponents. Isolated to pain-nutrition folder.
   ───────────────────────────────────────────────────────────────────────────── */
window.PNComponents = (function () {
  const icons = {
  ...
  function icon(n) { return icons[n] || icons.compass; }
  ...
  function renderHero({ eyebrow='', title='', sub='', primaryCTA=null, secondaryCTA=null }={}) {
    ...
    return `<section class="hero"><div class="hero-inner">
      ${eyebrow?`<div class="hero-eyebrow">${icon('heart')}${eyebrow}</div>`:''}
  ...
  function renderStatBlock(stats=[]) {
    return `<div class="stat-block">${stats.map(s=>`<div class="stat-item"><div class="stat-value">${s.value}</div><div class="stat-label">${s.label}</div></div>`).join('')}</div>`;
  }
  ...
  function renderCheckList(items=[]) {
    return `<ul class="check-list">${items.map(item=>`<li class="check-item"><div class="check-dot">${icon('check')}</div><span>${item}</span></li>`).join('')}</ul>`;
  }
  function renderCallout(text, variant='gold', iconName='heart') {
  ...
  function renderTrustStrip(items=[]) {
    return `<div class="trust-strip"><div class="trust-strip-inner">
      ${items.map((item,i)=>`<div class="trust-item">${icon(item.icon||'check')}<span><strong>${item.value}</strong> ${item.label}</span></div>${i<items.length-1?'<div class="trust-divider"></div>':''}`).join('')}
    </div></div>`;
  }
    ...
    return `<div class="footer-inner">
      <div class="footer-top">
        <div class="footer-brand">
          <a class="brand" href="#home" data-route="home"><span class="brand-mark">PN</span><span class="brand-name">Pain Nutrition</span></a>
          <p class="footer-brand-desc">Practical nutrition guidance for real people and real routines. Accessible, clear, and free from unnecessary complexity.</p>
        </div>
        <div class="footer-col"><div class="footer-col-title">Platform</div><ul class="footer-links">
          <li><a href="#features"      data-route="features">What We Offer</a></li>
          <li><a href="#safety"        data-route="safety">Our Approach</a></li>
          <li><a href="#accessibility" data-route="accessibility">Accessibility</a></li>
        </ul></div>
        <div class="footer-col"><div class="footer-col-title">Company</div><ul class="footer-links">
        ...
        <div class="footer-col"><div class="footer-col-title">Get Started</div><ul class="footer-links">
          <li><a href="#contact" data-route="contact">Coaching Enquiry</a></li>
          <li><a href="#contact" data-route="contact">Early Access</a></li>
          <li><a href="#partners" data-route="partners">Partner With Us</a></li>
        </ul></div>
      </div>
      <div class="footer-bottom">
        <p class="footer-legal">&copy; 2025 Pain Nutrition. Not medical advice. For educational purposes.</p>
        <p class="footer-legal"><a href="#contact" data-route="contact">Privacy</a>&nbsp;&middot;&nbsp;<a href="#contact" data-route="contact">Terms</a></p>
      </div>
    </div>`;
  }
  function initComponents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (link && mobileMenu) {
        mobileMenu.classList.remove('open'); hamburger && hamburger.classList.remove('open');
        hamburger && hamburger.setAttribute('aria-expanded', 'false');
        mobileMenu.setAttribute('aria-hidden', 'true');
      }
    });
  }
  return { icon, renderSectionHeader, renderHero, renderPageHero, renderCard, renderStatBlock,
    renderCTABanner, renderBadge, renderStepList, renderCheckList, renderCallout,
    renderFeatureRow, renderTrustStrip, renderPartnerCard, renderInfoRow, renderA11yItem,
