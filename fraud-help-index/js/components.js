/* ─────────────────────────────────────────────────────────────────────────────
   Fraud Help Index — Components
   Same API as VSTComponents. Isolated FHI namespace.
   ───────────────────────────────────────────────────────────────────────────── */

window.FHIComponents = (function () {
    ...
    return `
      <section class="hero">
        <div class="hero-inner">
          ${eyebrow ? `<div class="hero-eyebrow">${icon('shield')}${eyebrow}</div>` : ''}
          <h1 class="hero-title">${title}</h1>
          ${sub ? `<p class="hero-subtitle">${sub}</p>` : ''}
          ${(p || s) ? `<div class="hero-actions">${p}${s}</div>` : ''}
        </div>
      </section>`;
  }
    ...
    return `
      <div class="cta-banner">
        <div class="cta-banner-inner">
          ...
          <div class="hero-actions">${p}${s}</div>
        </div>
      </div>`;
  }
  ...
  function renderA11yItem({ title = '', desc = '' } = {}) {
    return `
      <div class="a11y-item">
        ...
        <div><div class="a11y-title">${title}</div><div class="a11y-desc">${desc}</div></div>
      </div>`;
  }

  function renderFraudTypeCard({ name = '', desc = '', action = '', risk = 'medium' } = {}) {
    const riskLabel = { high: 'High volume', medium: 'Common', low: 'Emerging' }[risk] || 'Common';
    return `
      <div class="fraud-type-card">
        <span class="fraud-type-badge badge-${risk === 'high' ? 'red' : risk === 'low' ? 'green' : 'medium'}">${riskLabel}</span>
        <div class="fraud-type-info">
          <div class="fraud-type-name">${name}</div>
          <div class="fraud-type-desc">${desc}</div>
          ${action ? `<div class="fraud-type-action">${action}</div>` : ''}
        </div>
      </div>`;
  }
        ...
        <div class="footer-top">
          <div class="footer-brand">
            <a class="brand" href="#home" data-route="home">
              <span class="brand-mark">FHI</span>
              <span class="brand-name">Fraud Help Index</span>
            </a>
            <p class="footer-brand-desc">The global directory for fraud reporting, recovery, and prevention. Independent. Free. Accessible.</p>
          </div>
          <div class="footer-col">
            <div class="footer-col-title">Resources</div>
            <ul class="footer-links">
              <li><a href="#fraud-types" data-route="fraud-types">Fraud Types</a></li>
              <li><a href="#prevention"  data-route="prevention">Prevention</a></li>
          ...
          <div class="footer-col">
            ...
            <ul class="footer-links">
              <li><a href="#contact" data-route="contact">Report Fraud Now</a></li>
              <li><a href="#contact" data-route="contact">Get Advice</a></li>
              <li><a href="#contact" data-route="contact">Victim Support</a></li>
            </ul>
          </div>
        </div>
        <div class="footer-bottom">
          <p class="footer-legal">&copy; 2025 Fraud Help Index. Independent. Not-for-profit. Free to use.</p>
  ...
  return {
    ...
    renderFraudTypeCard, renderFooter, initComponents,
  };
